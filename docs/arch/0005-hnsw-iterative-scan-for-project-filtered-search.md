...
- The cost is bounded latency, not unbounded: roughly break-even at low scale, up to ~2.3x p50 at the worst scale point measured (50 projects, 100 ADRs), still sub-4ms p50 in absolute terms. `PgStore.Search`'s query shape and `topK`/`similarity_threshold` semantics are unchanged -- iterative scan only affects how the graph walk satisfies them.
