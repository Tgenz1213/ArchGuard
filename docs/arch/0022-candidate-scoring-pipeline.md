...
- `stage.Scorer` receives a `File`, a `Debug`, and all of a file's `Candidate`s in one call, and returns one 0-1 score per candidate, in order. It only scores; it never drops or reorders.
...
- Every `VectorStore` implementation provides `ScopedADRs`.
