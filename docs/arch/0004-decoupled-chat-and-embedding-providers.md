...
- `cmd/archguard-e2e/main.go` supplies two distinct mock providers, one per role (`internal/cli.Execute`'s `chatProviderFactory`/`embedProviderFactory` parameters), reusing the chat mock for embedding only when `resolveEmbedProvider` says the two provider names match -- `test/e2e_dual_provider_test.go` exercises the dual-provider split through a real `archguard index` + `archguard check` subprocess run, not just at the unit level.
