...
`DefaultSystemPrompt` (`internal/llm/llm.go`) is deliberately narrow and "FALSE BY DEFAULT" -- it exists specifically to avoid false positives. Adding "and suggest a fix" to that same prompt risks subtly biasing the model toward finding violations, just to have something to suggest.
...
- `AnalysisResult.Suggestion` and `Violation.Suggestion` are both `omitempty` and populated only when a violation is confirmed and `--suggest-fixes` is set.
- Once computed, the suggestion is merged back into the cached `AnalysisResult` and re-persisted via the existing `internal/cache` mechanism (keyed on model/ADR/file/prompt, unchanged), so a later run of the same file/ADR pair reuses it without another LLM call.
...
- Positive: suggestion cost is paid at most once per (model, ADR, file-content) triple thanks to the cache merge, even across `--suggest-fixes` on/off toggling.
