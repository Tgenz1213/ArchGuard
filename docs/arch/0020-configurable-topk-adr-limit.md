...
`Engine.Run` reads this value once per `Run` call (alongside `concurrency`) and passes it to `stage.NewCosineStage`, which uses it as the cosine stage's `MaxKeep` (see `docs/arch/0022-candidate-scoring-pipeline.md`).
