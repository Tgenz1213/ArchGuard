...
- **Token counting can silently under/over-count for non-OpenAI models.** `Engine.fetchContext`'s tokenizer (`getTokenizer` in `internal/analysis/engine.go`) is a tiktoken encoder keyed off `LLM.Model`, falling back to `cl100k_base` for any model tiktoken doesn't recognize — which is every Ollama/Gemini model, including the `ollama`/`llama3.2` default install. This makes truncation boundaries approximate, not exact, for non-OpenAI providers. If you're touching truncation logic, check first whether `llm.Provider` has grown a `CountTokens` method (a fix moving truncation onto each provider's own tokenizer was in flight) — the fallback behavior described here may already be gone.
